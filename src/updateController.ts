...
const labels: Record<UpdateStatus, string> = {
  ...
  current: "Listo",
  "no-release": "Comprobar",
  available: "Descargar",
  downloading: "Descargando…",
  ready: "Reiniciar e instalar",
...
export class UpdateController {
  ...
  constructor(
    private adapter: UpdaterAdapter | null,
  ...
  private initialSnapshot(): UpdateSnapshot {
    ...
    return {
      ...
      detail: this.adapter
        ...
        : "Las actualizaciones están disponibles en la aplicación de Windows.",
      canAct: false,
      lastChecked: null,
      installing: false,
    };
  ...
  action = async (): Promise<void> => {
    if (!this.active || !this.adapter || this.running || this.snapshot.status === "installing")
      return;
    ...
    try {
      if (status === "ready" && this.update) {
        this.publish({
          status: "installing",
          detail: "Windows cerrará la aplicación para instalar y volver a abrirla.",
        });
        ...
        this.publish({
          status: "downloading",
          detail: "Descargando la actualización firmada…",
          progress: undefined,
        });
        let received = 0;
        ...
        await this.update.download(
          (event) => {
            ...
            const progress = total
              ...
              : undefined;
            if (event.event !== "Finished" && progress !== this.snapshot.progress)
        ...
        if (this.active && generation === this.generation)
          this.publish({
            status: "ready",
            progress: 100,
            detail: "Descarga verificada. Reinicia para instalar la actualización.",
          });
      } else {
        this.publish({
          status: "checking",
          detail: "Consultando las versiones publicadas…",
          progress: undefined,
        });
        ...
        if (this.active && generation === this.generation)
          this.publish({
            status: result ? "available" : "current",
            version: result?.version,
            lastChecked: Date.now(),
            detail: result
              ? `La versión ${result.version} está disponible.`
      ...
      if (this.active && generation === this.generation) {
        ...
        if (status === "ready") {
          this.publish({
            status: "ready",
            detail: "No se pudo iniciar el instalador. Puedes volver a intentarlo.",
          });
        } else if (status === "available") {
          this.publish({
            status: "available",
            progress: undefined,
            detail: "No se pudo descargar o verificar la actualización. Vuelve a descargarla.",
          });
        } else {
          ...
          const noManifest = /release.*not.*found|could not fetch a valid release json/i.test(
            message,
          );
          this.publish({
            status: explicit404 ? "no-release" : "error",
            lastChecked: Date.now(),
            detail: explicit404
              ? "Todavía no hay una versión publicada."
              : noManifest
                ? "No se encontró un manifiesto de actualización. Puede que aún no haya una versión publicada o que el servidor no esté disponible."
                : "No se pudo comprobar la actualización. Comprueba la conexión e inténtalo de nuevo.",
          });
        }
      }
    } finally {
