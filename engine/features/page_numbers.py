...
def run(inputs, output_dir, options):
    ...
    size = number(options, "font_size", 12, 1)
    if not start.is_integer(): raise ValueError("El número inicial debe ser entero.")
    writer = PdfWriter(clone_from=reader)
    for offset, index in enumerate(dict.fromkeys(pages(options.get("pages", ""), len(reader.pages)))):
        label = str(int(start) + offset)
        def draw(layer, width, height):
            if size + 24 >= height or layer.stringWidth(label, "Helvetica", size) >= width:
                raise ValueError("La numeración no cabe en la página.")
            layer.setFont("Helvetica", size)
            layer.drawCentredString(width / 2, 18, label)
        overlay_page(writer.pages[index], draw)
    return [save(writer, output_dir, "numerado.pdf")]
