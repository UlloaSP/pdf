...
def validate(options):
    integer(options, "x", 0)
    integer(options, "y", 0)
    integer(options, "width", 100, 1)
    integer(options, "height", 100, 1)


def transform(frame, count, options):
    x, y = integer(options, "x", 0), integer(options, "y", 0)
    width, height = integer(options, "width", 100, 1), integer(options, "height", 100, 1)
    if x + width > frame.width or y + height > frame.height:
        raise ValueError("El rectángulo de recorte debe quedar dentro de la imagen orientada.")
    check_size((width, height), count)
