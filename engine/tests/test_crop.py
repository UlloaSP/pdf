...
spec = importlib.util.spec_from_file_location("feature_under_test", Path(__file__).parents[1] / "features" / "crop.py")
...
class FeatureTest(unittest.TestCase):
    ...
    def test_real_document(self):
        ...
        reader = PdfReader(run([self.pdf], str(self.out), {'pages': '1'})[0])
        self.assertEqual(float(reader.pages[0].cropbox.width), 360)
        self.assertEqual(float(reader.pages[1].cropbox.width), 400)
