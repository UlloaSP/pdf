...
from engine.features import image_rotate as feature
...
class ImageTests(unittest.TestCase):
    ...
    def test_rejects_svg_and_bad_option(self):
        ...
        with self.assertRaises(ValueError): feature.run([str(self.source)],str(self.out),{'angle': 91})

    def test_orientation_filter(self):
        path=feature.run([str(self.source)],str(self.out),{'orientation':'Verticales'})[0]
        with Image.open(path) as image: self.assertEqual(image.size,(80,60))

    def test_clockwise_pixel_locations(self):
        for angle,pixel in [(90,(54,5)),(180,(74,54)),(270,(5,74))]:
            path=feature.run([str(self.source)],str(self.out/str(angle)),{'angle':angle})[0]
            with Image.open(path) as image:
                self.assertEqual(image.convert('RGBA').getpixel(pixel),(0,200,0,0))
