...
spec = importlib.util.spec_from_file_location("feature_under_test", Path(__file__).parents[1] / "features" / "edit.py")
...
class FeatureTest(unittest.TestCase):
    ...
    def test_real_document(self):
        ...
        reader = PdfReader(run([self.pdf], str(self.out), {'text':'Added text','pages':'1'})[0])
        self.assertIn('Added text', reader.pages[0].extract_text())
        self.assertIn('Page 1', reader.pages[0].extract_text())
        self.assertNotIn('Added text', reader.pages[1].extract_text())
        with self.assertRaises(ValueError): run([self.pdf], str(self.out), {'text':'x','x':999})
    ...
    def test_rejects_rotated_annotations(self):
        ...
        with self.assertRaisesRegex(ValueError,'anotaciones'):
            run([str(form)],str(self.out),{'text': 'Hello'})
