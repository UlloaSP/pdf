...
spec = importlib.util.spec_from_file_location("feature_under_test", Path(__file__).parents[1] / "features" / "rotate.py")
...
class FeatureTest(unittest.TestCase):
    ...
    def test_real_document(self):
        ...
        reader = PdfReader(run([self.pdf], str(self.out), {'pages': '2','angle': 90})[0])
        self.assertEqual(reader.pages[0].rotation, 0)
        self.assertEqual(reader.pages[1].rotation, 90)
        with self.assertRaises(ValueError): run([self.pdf], str(self.out), {'angle': 45})
