...
spec = importlib.util.spec_from_file_location("feature_under_test", Path(__file__).parents[1] / "features" / "watermark.py")
...
class FeatureTest(unittest.TestCase):
    ...
    def test_real_document(self):
        ...
        reader = PdfReader(run([self.pdf], str(self.out), {'text': 'CONFIDENTIAL','pages': '2'})[0])
        self.assertNotIn('CONFIDENTIAL', reader.pages[0].extract_text())
        self.assertIn('CONFIDENTIAL', reader.pages[1].extract_text())
        with self.assertRaises(ValueError): run([self.pdf], str(self.out), {'opacity': 2})
